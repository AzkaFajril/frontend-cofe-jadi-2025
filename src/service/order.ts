...
export async function getOrderById(id: string): Promise<DeliveryOrder | null> {
  try {
    const res = await fetch(`https://server-production-0205.up.railway.app/api/orders/${id}`);
...
export async function createOrder(orderData) {
  ...
  const res = await fetch('https://server-production-0205.up.railway.app/api/orders', {
    method: 'POST',
...
export async function getOrderHistoryByUser(userId: string) {
  const res = await fetch(`https://server-production-0205.up.railway.app/api/orders/by-user?userId=${userId}`);
