...
export async function getOrderById(id: string): Promise<DeliveryOrder | null> {
  try {
    const res = await fetch(`https://sekola-backend-production-bd7d.up.railway.app/api/orders/${id}`);
...
export async function getOrderHistoryByUser(userId: string) {
  const res = await fetch(`https://sekola-backend-production-bd7d.up.railway.app/api/orders/by-user?userId=${userId}`);
