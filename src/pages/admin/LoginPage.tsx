  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (typeof window.google === 'undefined') {
        alert('Google OAuth not available. Please make sure you are using a supported browser.');
        setLoading(false);
        return;
      }

      window.google.accounts.oauth2.initTokenClient({
        client_id: '756275400281-5cllputqbgk7j3kqjknki17uqj67bp6e.apps.googleusercontent.com',
        scope: 'https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile',
        callback: async (response) => {
          try {
            const tokenResponse = await fetch('https://sekola-backend-production-bd7d.up.railway.app/auth/google', {
              method: 'POST',
              headers: {
            ...
            if (tokenResponse.ok) {
              ...
              // Store user data and role for admin authentication
              ...
              if (data.user.role === 'admin') {
                ...
                // Jika user bukan admin, redirect ke home dan tampilkan pesan
                alert('Access denied. Only admin users can access this area.');
