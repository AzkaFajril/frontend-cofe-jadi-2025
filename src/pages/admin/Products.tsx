...
const Products: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch('https://sekola-backend-production-bd7d.up.railway.app/admin/products', {
        headers: {
  ...
  const handleSubmit = async (formData: ProductFormData) => {
    try {
      ...
      const url = editingProduct 
        ? `https://sekola-backend-production-bd7d.up.railway.app/admin/products/${editingProduct._id}`
  ...
  const handleDelete = async (productId: string) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://sekola-backend-production-bd7d.up.railway.app/admin/products/${productId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
