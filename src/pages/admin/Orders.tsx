...
const Orders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch('https://server-production-0205.up.railway.app//admin/payments', {
        headers: {
  ...
  const handleStatusUpdate = async (orderId: string, newStatus: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://server-production-0205.up.railway.app//admin/orders/${orderId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
              ...
              {orders.map((order, index) => (
                <tr key={order._id || `order-${index}`} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    #{order._id?.slice(-6) || 'Unknown'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm font-medium text-gray-900">{order.userId?.name || 'Unknown User'}</div>
