...
const Users: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch('https://sekola-backend-production-bd7d.up.railway.app/admin/users', {
        headers: {
  ...
  const handleDeleteUser = async (userId: string) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://sekola-backend-production-bd7d.up.railway.app/admin/users/${userId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
  ...
  const handleSubmitEditUser = async (formData: Partial<User>) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://sekola-backend-production-bd7d.up.railway.app/admin/users/${editingUser._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
